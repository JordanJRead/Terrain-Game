...
void DeferredRenderer::doDeferredShading(const FramebufferColour* const targetFramebuffer, const TerrainImageSet& terrainImageSet, const VertexArrayScreenQuad& screenQuad, const ShadowMapper<CascadeCount>& shadowMapperSun, const ShadowMapper<CascadeCount>& shadowMapperMoon, const glm::ivec2& debugFragPos) {
	...
	mShaderWaterDeferred.render(targetFramebuffer, screenQuad, mShadedTerrainFramebuffer.getColourTex(0), mGeometryFramebuffer, mBlueNoise, shadowMapperSun, shadowMapperMoon, debugFragPos); // Will copy shaded terrain buffer too TODO add src texture?
